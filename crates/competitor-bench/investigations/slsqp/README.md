# SLSQP temporary-allocation investigation
...
The remaining gap is approximately 39% against the Rust reference. A supported
next experiment is reusing the remaining per-iteration QP/BFGS work buffers,
with constrained workloads and allocation counts measured separately. That
requires more workspace/lifecycle changes than these local copy eliminations.
