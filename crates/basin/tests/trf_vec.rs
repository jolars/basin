//! Trust-region-reflective over the default `Vec<f64>` backend (no feature
//! gate). Mirrors `tests/trf_nalgebra.rs`: the hand-rolled
//! [`DenseMatrix`](basin::DenseMatrix) supplies `GramMatrix`,
//! `AddDiagonalVectorInPlace`, the pure-Rust Cholesky `LinearSolveSpd`, and
//! `MaxDiagonal` (the last op that previously kept TRF off `Vec<f64>`), so the
