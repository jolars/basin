...
- `src/core.rs` + `src/core/`: the framework:
  ...
  - `numdiff.rs`: the `FiniteDiff` wrapper: synthesizes `Gradient` / `Jacobian`
    / `Hessian` from function values via finite differences.
...
4. **First-class constraints.** Constraints describe the *problem*, so they live
   ...
   compile error, with opt-in adapters (projection / barrier / penalty) to wrap
