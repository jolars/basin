...
- [ ] **Unified `Composed<Outer, Inner>` abstraction (or honest "no").** Two
  ...
  that is explicitly **not** a `Composed` abstraction (it says nothing
...
- [x] **State construction surface: hide the state zoo behind the solver (or
  ...
  (a): coexistence). CMA-ES (needs σ), the population GA, DE, or
  ...
  which termination criteria bind on per tenet 3): keep unconditionally—a
  criterion *reads* its data off the state, so binding `GradientTolerance`
  ...
  worsen error messages and erase the "why is L-BFGS different" signal—don't.
  **Layer 3—construction is the only real friction.** The zoo is
  ...
  **Purely additive** (the three-arg form stays for custom initial
  ...
  before reaching for it: (a) explicit state construction is not pure cost—it's
  where `x0` lives and it makes the solver↔state pairing visible,
