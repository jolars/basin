<script lang="ts">
    ...
    const g = $derived.by(() => {
        ...
        const allNs = series
            .flatMap((s) => s.points.map((p) => p.ns))
            .filter((v) => v > 0);
        const yLo = Math.floor(Math.log10(Math.min(...allNs)));
        const yHi = Math.max(
            Math.ceil(Math.log10(Math.max(...allNs))),
            yLo + 1,
        );
        const ySpan = yHi - yLo;

        const xPx = (n: number) =>
            padL + ((Math.log10(n) - xMin) / xSpan) * innerW;
        const yPx = (ns: number) =>
            axisY - ((Math.log10(ns) - yLo) / ySpan) * innerH;
        ...
        const ends = lines
            ...
            .map((l) => {
                ...
                const ang = Math.max(
                    ...
                    Math.min(
                        tiltCap,
                        Math.atan2(end.cy - prev.cy, end.cx - prev.cx),
                    ),
                );
                return {
                    text: l.label,
