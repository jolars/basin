...
- **`LinearInequalityConstraints`** (`A x ≤ b`, exposing `a()` and `b()`): kept
  ...
  Wolfe or More-Thuente inner can step through it; pair `Bfgs`/`Lbfgs` with
...
- **`NonlinearInequalityConstraints`** (`c(x) ≤ 0` for an arbitrary
  ...
  feasible by an *exact-penalty merit function with a geometry and acceptance test*
  ...
  matrix or vector data, so the trait carries an evaluator rather than `a()`/`b()`
...
`NonlinearConstraints` *aggregator* (folding nonlinear + linear-ineq and eq + box
...
*exact-penalty merit + geometry and acceptance test* (COBYLA, a derivative-free
...
the inequality block, silently dropping any box or equality data the problem also
...
PRIMA's full COBYLA folds nonlinear inequalities + linear ineq and eq + box bounds
...
would silently drop linear or box data). Adding it later is purely additive and
